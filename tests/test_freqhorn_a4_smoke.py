...
class FreqHornA4SmokeTest(unittest.TestCase):
    def test_freqhorn_emits_payload_for_a4(self):
        repo_root = Path(__file__).resolve().parents[3]
        binary = repo_root / "build" / "tools" / "deep" / "freqhorn"
        benchmark = repo_root / "pwa-horn-benchmarks" / "possible_features" / "algebraic_numbers" / "a4.smt2"
        ...
        self.assertTrue(any(entry.get("period") == 8 for entry in pair_entries), pair_entries)
