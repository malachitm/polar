...
class FreqHornMixedAlgebraicPhaseSmokeTest(unittest.TestCase):
    def test_freqhorn_emits_mixed_aux_and_phase_payload(self):
        repo_root = Path(__file__).resolve().parents[3]
        binary = repo_root / "build" / "tools" / "deep" / "freqhorn"
        benchmark = repo_root / "pwa-horn-benchmarks" / "benchmarks" / "mixed_alg_phase.smt2"
